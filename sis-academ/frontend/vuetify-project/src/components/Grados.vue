<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat class="rounded">
        <v-toolbar-title>
          <v-icon start>mdi-chevron-triple-up</v-icon>
          Gestión de Grados
        </v-toolbar-title>
        <v-text-field
          v-model="search"
    ...
    <template v-slot:item.acciones="{ item }">
      <div class="d-flex ga-2">
        ...
        <v-icon
          ...
          icon="mdi-delete"
          size="small"
          @click="handleConfirm(item.idGrado)"
        ></v-icon>
      </div>
    </template>
  </v-data-table>
  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card class="pb-5 pl-5 pr-5" :title="`${editing ? 'Editar' : 'Agregar'} un Grado`">
  ...
  <v-dialog v-model="confirmDialog.enabled" max-width="500">
    <v-card :title="`¿Eliminar ${selected.length === 0 ? 'el Grado' : 'todos los Grados'}?`" 
...
const API_URL_MATERIAS_GRADOS = "http://localhost:8080/api/grados/materias-grados";
const items = ref([]);
const loading = ref(true);
const selected = ref([]);
const headers = [
  { title: "ID", key: "idGrado" },
...
async function remove(id) {
  await axios
    .delete(`${API_URL_GRADOS}/delete/${id}`)
    .then((res) => {
...
async function removeSelected() {
  ...
  try {
    await Promise.all(
      selected.value.map((grado) =>
        axios.delete(`${API_URL_GRADOS}/delete/${grado.idGrado}`)
      )
    );
    showSnackbar("Grados eliminados correctamente");
    selected.value = [];
  } catch (error) {
    showSnackbar("Error al eliminar los grados", "error");
  }
  fetch();
  confirmDialog.value.enabled = false;
  confirmDialog.value.id = null;
...
async function save() {
  if (editing.value) {
    await axios
      .put(`${API_URL_GRADOS}/update/${record.value.idGrado}`, record.value)
      .then((res) => {
        showSnackbar(res.data);
        dialog.value = false;
      })
    ...
    await axios
      .post(`${API_URL_GRADOS}/create`, record.value)
      .then((res) => {
        showSnackbar(res.data);
        dialog.value = false;
      })
